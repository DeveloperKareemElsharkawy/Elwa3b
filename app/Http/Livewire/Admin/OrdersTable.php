...
final class OrdersTable extends PowerGridComponent
{
    ...
    */
    ...
    {
        return Order::query()->with('processes.process')
            ...
            ->join('workers', function ($worker) {
                $worker->on('orders.worker_id', '=', 'workers.id');
            })
            ->join('users', function ($user) {
                $user->on('orders.user_id', '=', 'users.id');
            })->select('orders.*', 'workers.name as worker_name', 'users.name as user_name');
    }
    ...
    public function addColumns(): ?PowerGridEloquent
    {
        return PowerGrid::eloquent()
            ->addColumn('client', function (Order $order) {
                return '<div class="pro-user-username mb-1 font-weight-bold"><a class="btn-link fs-16" target="_blank"href=" ' . route('admin.users.show', ['user_id' => $order->user_id]) . '  ">' . $order->user->name . '</a>  </div>' . '<br>' . '<span class="avatar avatar-xxl bradius" style="background-image: url(' . asset('storage/' . $order->user->image) . ')"></span>';
            })
            ->addColumn('worker', function (Order $order) {
                return '<div class="pro-user-username mb-1 font-weight-bold"><a class="btn-link fs-16" target="_blank"href=" ' . route('admin.workers.show', ['worker_id' => $order->worker_id]) . '  ">' . $order->worker->name . '  </div>' . '<br>' . '<span class="avatar avatar-xxl bradius" style="background-image: url(' . asset('storage/' . $order->worker->face_image) . ')"></span>';
            })
            ->addColumn('nationality_processes', function (Order $order) {
                $processes = '';
                foreach ($order->processes as $orderProcess) {
                    $processes .= view('admin-panel.orders.partials.processes', compact('orderProcess'));
                }
    ...
    */
    ...
    public function columns(): array
    {
        return [
            ...
            Column::make(trans('admin/datatable.worker'), 'worker', 'workers.name')
                ->makeInputMultiSelect(Worker::query()->whereHas('orders')->get(), 'name', 'worker_id')
                ->sortable(),

            Column::add()
                ->title(trans('admin/datatable.order_status'))
                ->field('status')
                ->searchable()
                ->makeInputEnumSelect(OrderStatus::cases(), 'orders.status')
                ->sortable(),
            Column::add()
                ->title(trans('admin/datatable.order_nationality_processes'))
                ->field('nationality_processes')
                ->searchable()
                ->sortable(),
