...
final class WorkerArchivesTable extends PowerGridComponent
{
    use ActionButton;

    //Messages informing success/error data is updated.
    public bool $showUpdateMessages = true;
    public array $workersArchivesIdsForDeletion = [];
    ...
    */
    ...
    {
        return WorkerArchives::query()->where('worker_id', $this->worker_id);
    }
    ...
    */
    public function addColumns(): ?PowerGridEloquent
    {
        return PowerGrid::eloquent()
            ...
            ->addColumn('created_at_formatted', function (WorkerArchives $model) {
    ...
    */
    ...
    /**
     * PowerGrid WorkerArchives Action Buttons.
     *
    ...
    public function actions(): array
    {
        return [
            ...
            Button::add('edit')
                ...
                ->target('_self')
                ->route('admin.workers_archives.edit', ['worker_archive_id' => 'id', 'worker_id' => $this->worker_id]),
            ...
            Button::add('destroy')
                ...
                ->target('_self')
                ->emit('ConfirmRemoveWorkersArchives', ['workers_archives_ids' => 'id']),
        ];
    }
    ...
    public function header(): array
    {
        ...
        return [
            Button::add('bulk-sold-out')
                ->caption($CreatNewBtnIcon . __('admin/datatable.buttons.c_worker_archive'))
                ->class('btn btn-success  btn-svgs btn-svg-white datatable-delete-btn')
                ->target('_self')
                ->route('admin.workers_archives.create', ['worker_id' => $this->worker_id]),
            ...
            Button::add('bulk-sold-out')
                ...
                ->target('_self')
                ->emit('ConfirmRemoveWorkersArchives', []),
        ];
    }
    ...
    protected function getListeners()
    {
        return array_merge(
            parent::getListeners(), [
            'alertEvent',
            'ConfirmRemoveWorkersArchives',
            'deleteConfirmed' => 'deleteWorkersArchives',

        ]);
    }

    public function ConfirmRemoveWorkersArchives($data)
    {
        if (count($this->checkboxValues) == 0 && !isset($data['workers_archives_ids'])) {
            $this->dispatchBrowserEvent('NoSelectedItemsAlert', ['message' => 'You must select at least one item!']);
            return;
        }

        $systemUsersIds = isset($data['workers_archives_ids']) ? [$data['workers_archives_ids']] : $this->checkboxValues;

        $this->workersArchivesIdsForDeletion = $systemUsersIds;

        $this->dispatchBrowserEvent('show-delete-confirmation', ['message' => 'You have selected IDs: ']);
    }

    public function deleteWorkersArchives()
    {
        $ids = array_map(function ($value) {
            return intval($value);
        }, $this->workersArchivesIdsForDeletion);
        WorkerArchives::query()->whereIn('id', $ids)->delete();
        $this->workersArchivesIdsForDeletion = [];
    }
    ...
    */
    ...
    /**
     * PowerGrid WorkerArchives Action Rules.
     *
    ...
    public function actionRules(): array
    {
       return [
           ...
           //Hide button edit for ID 1
            Rule::button('edit')
                ->when(fn($worker-archives) => $worker-archives->id === 1)
                ->hide(),
        ];
    }
    */
    ...
    */
    ...
    /**
     * PowerGrid WorkerArchives Update.
     *
